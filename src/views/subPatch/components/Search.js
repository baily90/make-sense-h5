...
import { getSubPatchList } from '../../../redux/actionsAsync/subPatch';
import { setSearchParamsAction } from '../../../redux/actions/subPatch';
...
const Search = () => {
  ...
  const params = useSelector((state) => state.subPatch.searchParams);
  const [form] = Form.useForm();

  useEffect(() => {
    const searchParamsAction = setSearchParamsAction({ searchParams: defaultSearchParams });
    dispatch(searchParamsAction);
    const engineerList = getSubPatchList({ params: defaultSearchParams });
  ...
  const onFinish = (values) => {
    ...
    dispatch(searchParamsAction);
    const engineerList = getSubPatchList({ params: searchParams });
    dispatch(engineerList);
  };
  const onReset = () => {
    form.resetFields();
